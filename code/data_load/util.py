...
def augment(img_list, hflip=True, rot=True):
    ...
    def _augment(img):
        if hflip:
            idx = [i for i in range(img.size(3)-1, -1, -1)]
            img = img[:, :, :, idx]
            # print('hflip')
        if vflip:
...
def random_crop(raw, com, patch_size_lr, scale_factor):  # raw: N*H*W
    _, _, h_hr, w_hr = raw.shape
    ...
    raw = raw[:, :, h_start:h_end, w_start:w_end]
    com = com[:, :, h_start_lr:h_end_lr, w_start_lr:w_end_lr]
